...
import { ArrowLeft, Clock3, GitBranchPlus, MessageSquareText } from "lucide-react";
...
import { getAvailableTransitions, getContentItemById } from "@/lib/marketing-data";
...
export default async function ItemDetailPage({
  ...
  return (
    <div className="app-shell page-grid">
      <Link href="/queue" className="inline-flex items-center gap-2 text-sm font-medium" style={{ color: "var(--text-secondary)" }}>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <section className="surface rounded-[1.75rem] p-5">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em]" style={{ background: "rgba(16, 37, 42, 0.06)", color: "var(--text-secondary)" }}>
              {item.scheduledDate ?? "Unscheduled"}{item.suggestedTime ? ` · ${item.suggestedTime}` : ""}
            </span>
          </div>
          ...
          <div className="mt-6 rounded-[1.5rem] border p-5" style={{ borderColor: "var(--border)" }}>
            ...
            <div className="mt-5 whitespace-pre-wrap text-base leading-8">{item.currentVersion.body || item.currentVersion.excerpt}</div>
          ...
          {item.currentVersion.imageUrl || item.selectedAsset?.assetUrl ? (
            <div className="mt-6 rounded-[1.5rem] border p-5" style={{ borderColor: "var(--border)" }}>
              ...
              <div className="mt-5 overflow-hidden rounded-[1.2rem] border" style={{ borderColor: "var(--border)" }}>
                <Image
              ...
              {item.currentVersion.imagePrompt ? (
                <p className="mt-4 text-sm leading-7" style={{ color: "var(--text-secondary)" }}>
                  Prompt: {item.currentVersion.imagePrompt}
                </p>
              ) : null}
            </div>
          ) : null}
        ...
        <section className="space-y-4">
          ...
          <div className="surface rounded-[1.75rem] p-5">
            ...
            <div className="mt-4 space-y-3 text-sm leading-7" style={{ color: "var(--text-secondary)" }}>
              ...
              <p>Last review: {item.review.createdAt}</p>
              <p>Revision cycles: {item.revisionCount ?? 0}</p>
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              {item.state === "ready_to_post" ? <MarkPostedButton item={item} /> : null}
            </div>
          </div>
